...
public enum HookInjection {
    ...
    public static let notifierScript = """
        #!/bin/sh
        # Orchestrate notifier - forwards agent events to the app's Unix socket.
        # Usage: orchestrate-notify.sh <kind>            (payload JSON on stdin)
        #        orchestrate-notify.sh <kind> <payload>  (payload JSON as argument)
        # No-op when the Orchestrate app is not running (socket absent).
        [ -n "$ORCHESTRATE_SOCK" ] && [ -S "$ORCHESTRATE_SOCK" ] || exit 0
        if [ "$#" -ge 2 ]; then payload="$2"; else payload="$(cat 2>/dev/null)"; fi
        [ -n "$payload" ] || payload=null
        printf '{"session_id":"%s","kind":"%s","payload":%s}' \\
            "$ORCHESTRATE_SESSION_ID" "${1:-unknown}" "$payload" \\
            | /usr/bin/nc -U "$ORCHESTRATE_SOCK" -w 2 >/dev/null 2>&1
        exit 0
        """
    ...
    /// Merges the Orchestrate `Stop` / `Notification` hooks into an existing
