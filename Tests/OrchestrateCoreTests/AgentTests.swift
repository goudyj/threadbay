...
private func makeTempDir() throws -> URL {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("orchestrate-agent-tests-\(UUID().uuidString)", isDirectory: true)
...
final class HookInjectionTests: XCTestCase {
    ...
    func testInstallsExecutableNotifier() throws {
        ...
        XCTAssertTrue(content.contains("ORCHESTRATE_SOCK"))
    }
...
final class EventSocketServerTests: XCTestCase {
    func testReceivesOneEventPerConnection() throws {
        let path = "/tmp/orchestrate-test-\(UUID().uuidString.prefix(8)).sock"
