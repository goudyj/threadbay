...
final class YamlInteropTests: XCTestCase {
    /// The exact shape the CLI writes today (`orchestrate settings`).
    private let settingsYAML = """
        default_project: orchestrate
        projects:
        - name: orchestrate
          path: /Users/jean/Documents/developpement/orchestrate
          files_to_include: []
        agent_commands:
    ...
    private let spacesYAML = """
        spaces:
        - project_name: orchestrate
          destination: /Users/jean/Documents/developpement/orchestrate__feature-x
          name: orchestrate__feature-x
    ...
    func testDecodeSettings() throws {
        ...
        XCTAssertEqual(settings.defaultProject, "orchestrate")
        XCTAssertEqual(settings.projects.count, 1)
        XCTAssertEqual(settings.projects[0].name, "orchestrate")
        XCTAssertEqual(settings.projects[0].path, "/Users/jean/Documents/developpement/orchestrate")
    ...
    func testSettingsRoundTripPreservesAgentCommands() throws {
        ...
        XCTAssertEqual(again.defaultProject, "orchestrate")
    ...
    func testDecodeSpaces() throws {
        ...
        XCTAssertEqual(space.projectName, "orchestrate")
        XCTAssertEqual(space.name, "orchestrate__feature-x")
